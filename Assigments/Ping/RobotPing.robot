...
Ping Websites
    ...
    FOR    ${url}    IN    @{webList}
        ...
        #ping-time-1
        ${ip}=    Evaluate    ${ip}+3                                    #get the index of the ping time
        ${ping-time}=   Set Variable   ${WordList}[${ip}]                #set the value based on ip index
        ${ping-time}=    Remove String    ${ping-time}    time=          #remove the string 'time='
        Should Be True    ${ping-time}<50                                #check if the ping time is less than 50ms
        Append To File    ${path}ping-result.txt    Ping Time: ${ping-time}\n    #write the ping time to the file
    ...
    #Append the average ping time to the file
